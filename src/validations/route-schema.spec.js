...
describe('routeSchema', () => {
  ...
  describe('campos obligatorios', () => {
    ...
    it('debería requerir origen', () => {
      const result = routeSchema.safeParse({ ...validRoute, origin: '' })
      expect(result.success).toBe(false)
    })

    it('debería requerir destino', () => {
      const result = routeSchema.safeParse({ ...validRoute, destination: '' })
      expect(result.success).toBe(false)
    })
  ...
  describe('status', () => {
    ...
    it('debería aceptar active', () => {
      const result = routeSchema.safeParse({ ...validRoute, status: 'active' })
      expect(result.success).toBe(true)
    })
    ...
    it('debería usar planned por defecto', () => {
      const result = routeSchema.safeParse(validRoute)
      expect(result.success).toBe(true)
  ...
  describe('cargo_type', () => {
    it('debería aceptar general', () => {
      const result = routeSchema.safeParse({ ...validRoute, cargo_type: 'general' })
      expect(result.success).toBe(true)
    })

    it('debería aceptar dangerous', () => {
      const result = routeSchema.safeParse({ ...validRoute, cargo_type: 'dangerous' })
      expect(result.success).toBe(true)
    })

    it('debería rechazar tipo inválido', () => {
      const result = routeSchema.safeParse({ ...validRoute, cargo_type: 'invalid' })
      expect(result.success).toBe(false)
    })
  })
  ...
  describe('campos opcionales completos', () => {
    it('debería aceptar ruta con todos los campos', () => {
      const result = routeSchema.safeParse({
        ...
        real_distance_km: 635,
        planned_duration_hours: 6,
        real_duration_hours: 6.5,
