...
describe('document-cmr', () => {
  ...
  it('debería ser una función exportada', () => {
    expect(typeof generateCmrDocument).toBe('function')
  })

  it('debería generar documento CMR con routeId', async () => {
    const result = await generateCmrDocument({ routeId: 'route-1' })
    expect(result).toBeDefined()
    expect(result).toHaveProperty('url')
    expect(result).toHaveProperty('documentId')
    expect(result).toHaveProperty('filename')
    expect(result.filename).toMatch(/cmr.*\.pdf$/)
  })
})
